...
export default function CaseHeader({ caseId, title, status = 'pending', statusLabel, actions, children }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-4 border-b border-phx-border/50 mb-6">
      <div className="flex flex-col gap-1.5">
        {caseId && <div className="font-mono text-xs font-bold text-phx-cyan uppercase tracking-widest">{caseId}</div>}
        <div className="font-serif text-2xl font-semibold text-phx-primary">{title}</div>
      </div>
      <div className="flex items-center gap-3 self-start sm:self-end">
        <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-sans font-medium whitespace-nowrap ${config.className}`}>
